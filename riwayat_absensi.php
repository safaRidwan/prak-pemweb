        ...
        <div class="card shadow-sm">
          <div class="card-body">
              <div class="d-flex justify-content-between align-items-center mb-4">
                  <h5 class="card-title fw-semibold mb-0">Riwayat Absensi Anda</h5>
              </div>
              ...
              <div class="table-responsive">
                  <table class="table table-striped table-hover align-middle">
                      ...
                      <tbody>
                          <?php
                          // PERUBAHAN QUERY: Menggunakan tabel kegiatan sebagai patokan utama, 
                          // lalu digabungkan (LEFT JOIN) dengan data presensi milik user yang login.
                          $query = mysqli_query($conn, "
                              SELECT k.nama_kegiatan, k.waktu_mulai, p.jam_masuk, p.jam_keluar 
                              FROM kegiatan k 
                          ...
                          if(mysqli_num_rows($query) > 0){
                              while($row = mysqli_fetch_assoc($query)){
                                  // Set default value jika data jam kosong (karena user belum/tidak absen)
                                  $jam_masuk = !empty($row['jam_masuk']) ? $row['jam_masuk'] : '-';
                                  $jam_keluar = (!empty($row['jam_keluar']) && $row['jam_keluar'] != '00:00:00') ? $row['jam_keluar'] : '-';
                                  
                                  // Logika status badge
                                  if($jam_masuk == '-'){
                                      $badge = '<span class="badge bg-danger">Tidak Absen</span>';
                                  } else if ($jam_keluar == '-') {
                                      $badge = '<span class="badge bg-warning text-dark">Belum Keluar</span>';
                                  } else {
                          ...
                          <?php 
                              ...
                              echo "<tr><td colspan='5' class='text-center'>Belum ada kegiatan yang terdaftar.</td></tr>";
                          }
                          ?>
                      </tbody>
                  </table>
